...
- 盘中主线/换股候选链路新增“行业 + 概念”联合题材判断：`market_monitor.py` 和 `rebalance_engine.py` 现在会把概念板块资金榜、趋势候选和盘前副龙头扫描合并起来，额外标注“板块领涨龙头 / 副龙补涨候选 / 分歧低吸 / 已脱离低吸窗口”等时机标签，减少商业航天、军工这类概念主线切换被漏判，以及报告继续给过期低吸价的情况。

- 飞书持仓口令链路新增“交易反馈回灌”能力：本地模型现在除了识别加仓/减仓/清仓/查看持仓，也会识别类似 `7.61清仓后立刻被拉升到了7.77` 这类复盘反馈；反馈会落到 `scanner_history.db` 的 `trade_feedback` 表，并在后续 `调仓` / 多模型辩论 prompt 中以“实盘反馈纠偏”区块重新注入，帮助系统纠正卖飞、追高、左侧接早等执行偏差。
- `portfolio_bot.py` 新增“批量持仓同步 + 明日计划识别”兜底：像 `协鑫集成现在是5.24元13手，赤天化4.3加仓3手，雷科防务13.256 2手` 这类混合消息会拆成多条持仓变更/反馈分别执行；而 `明天赤天化清仓` 这类口令只会识别为未来计划并明确提示“不修改当前持仓”，避免误以为已落账。
