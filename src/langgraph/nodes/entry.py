...
INTENT_PATTERNS = [
    ...
    (r"^(停止交易|急停|紧急停止|关闭交易|关闭自动交易|停止自动交易)$", "broker_halt"),
    (r"^(恢复交易|恢复下单|解除停止|开启交易|开启自动交易)$", "broker_resume"),
    (r"^(关闭买入|停止买入|关闭自动买入|禁止买入|不买|不要买)$", "broker_halt_buy"),
    (r"^(开启买入|恢复买入|打开买入|允许买入|可以买)$", "broker_resume_buy"),
    (r"^(关闭卖出|停止卖出|关闭自动卖出|禁止卖出|锁仓|不卖)$", "broker_halt_sell"),
    (r"^(开启卖出|恢复卖出|打开卖出|允许卖出|解锁)$", "broker_resume_sell"),
    (r"^(交易状态|开关状态|自动交易状态|自动交易|交易开关)$", "broker_status"),
]
