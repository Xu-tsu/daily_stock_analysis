"""
华尔街之狼 事件驱动+盲态Agent回测 V5
==========================================
无未来函数 | 无预设标签 | 事件冲击检测 + 自适应仓位

核心创新:
  Agent不知道"市场是牛还是熊"，也不知道"发生了什么新闻"
  但Agent能从价格数据中观察到:
    1. 今天有大量股票同时暴跌 → 大概率有重大事件(如关税/地缘冲突)
    2. 某组股票走势高度相关且一起暴跌 → 属于同一板块
    3. 暴跌后1-3天出现缩量企稳 → 恐慌结束，反弹机会

  这比"人为判断牛熊"更接近真实实盘:
    - 实盘中看到特朗普推文 → 程序检测到石油股集体暴跌 → 同样的信号
    - 不需要NLP理解新闻，价格已经包含了一切信息

信号层次:
  L1. 常规信号: 个股技术面评分(低吸/突破/回踩/接力)
  L2. 事件信号: 检测到市场冲击 → 冲击后超跌个股加分
  L3. 恐慌信号: 全市场恐慌(跌停家数暴增) → 全面防守，只做极端超跌
...
class EventDetector:
    ...
    def __init__(self):
        ...
        self._recent_shocks: List[dict] = []  # 近期冲击记录

    def analyze(self, all_data: dict, date: str) -> dict:
        """分析当日市场状态，返回事件信号"""
        total = 0
        down_3pct = 0      # 跌幅>3%家数
        down_5pct = 0      # 跌幅>5%家数
...
class EventAgentBacktest:
    ...
    def run(self, start_date, end_date):
        ...
        logger.info(f"Event Agent V5: {start_date} ~ {end_date} ({len(days)}d)")
        ...
        for day_num, date in enumerate(days):
            ...
            # 4. 盘后: 事件分析
            event = self._event_detector.analyze(self.all_data, date)
    ...
    def _review_sells(self, date, day_idx):
        ...
        for code, h in list(self.holdings.items()):
            ...
            # 事件驱动买入的快进快出: 2天没赚就跑
            if h.signal_type == "event_recovery" and hold >= 2 and pnl < 1:
    ...
    def _scan_event_recovery(self, date, day_idx, event):
        ...
        if not recovering_from: return

        # 获取冲击时跌幅大的股票
        shocked_codes = set(recovering_from.get("down_codes", []))
        if not shocked_codes: return

        pmin, pmax = RISK_PARAMS["price_range"]
        threshold = max(30, self._state.buy_threshold - 10)  # 事件恢复降低门槛

        cands = []
        for code in shocked_codes:
    ...
    def _sell(self, code, shares, price, date, reason):
        ...
        self.trades.append(Trade(date, code, h.name, "sell", shares, sp, amt, comm + tax, reason))
    ...
    def _print_report(self):
        ...
        event_trades = sum(1 for t in self.trades if "event_recovery" in t.reason or "EVT_" in t.reason)

        print("\n" + "=" * 70)
        print("  Event-Driven Blind Agent V5 Backtest Report 2026 YTD")
        print("  (No labels, self-adaptive, event shock detection)")
        print("=" * 70)
        ...
        print(f"  Event trades: {event_trades}")
        ...
        for e in self._event_log:
            print(f"    {e['date']} {e['level']:<10} panic={e['panic']:>2} LD={e['LD']} LU={e['LU']}")
        ...
        # 保存
        out_dir = Path("data/backtest_event_agent")
